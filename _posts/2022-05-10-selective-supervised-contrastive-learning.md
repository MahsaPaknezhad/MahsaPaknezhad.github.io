...
In supervised CL [4], two augmentation operations are applied to each example in the input batch of data to generate two different views for that example. Both views are passed through an encoder network and the output representations are L2-normaliezed. Positives for an example consist in the representations that correspond to the same batch instance or to any other instances with the same label as the example. Negatives for an example are all the remaining instances.  The representations are passed through a projection network as is common in CL algorithms to learn meaningful features. A supervised contrastive loss is imposed on the normalized outputs of the projection network [5]. 
