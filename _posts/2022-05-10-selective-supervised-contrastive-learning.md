...
$A(i)$ specifies the set of indices excluding $i$ and $\mathcal{G}_i = \{g \in A(i), P_{i'g'} \in \mathcal{G}\}$ where $i'$ and $g'$ are the original indices of $x_i$ and $x_g$. $\tau \in \mathbb{R}^+$ is a temperature parameter. 
