...
------
Since there exists no labels when training the CL algorithm with an unlabeled dataset, a positive pair is generated by pairing a randomly selected example and an augmentation of the same example. Negative pairs are generated by pairing an example with any other example in the training dataset. Fasle negatives generated by samples from the same class can degrade the learned representations. Using label information, positive pairs can be generated by pairing examples from the same class and negative examples can be generated by pairing examples from different classes. 

In supervised CL [1], two augmentation operations are applied to each example in the input batch of data to generate two different views for that example. Both views are passed through an encoder network and the output representations are L2-normaliezed. Positives for an example consist in the representations that correspond to the same batch instance or to any other instances with the same label as the example. Negatives for an example are all the remaining instances.  The representations are passed through a projection network as is common in CL algorithms to learn meaningful features. A supervised contrastive loss is imposed on the normalized outputs of the projection network [2]. 
...
[2] Google Blog: https://ai.googleblog.com/2021/06/extending-contrastive-learning-to.html?m=1
